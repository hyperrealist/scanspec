"""An example_spec directive for v2 specs."""
...
class ExampleSpecDirective(plot_directive.PlotDirective):
    """Runs `plot_scan` on the ``spec`` defined in the content.

    ``plot_scan`` (not 1.x's ``plot_spec``) since it accepts a bare Spec
    directly -- auto-compiling and, since there's no explicit ``spec=``
    override here, also using it for any region-boundary overlay -- rather
    than requiring the caller to compile it first.
    """

    def run(self) -> Any:
        """Run the directive."""
        self.content = StringList(
            ["# Example Spec", "", "from scanspec.v2.plot import plot_scan"]
